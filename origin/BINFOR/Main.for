...
C#################################################################    
       ...
       CLOSE(600)        
        ...
        CALL PRE2                                   !读入地形前处理文件       
        CALL Take_Boundary_for_TwoD                 !读入边界条件

	  IF (ICON.EQ.0) THEN                            !输出不同方案结果
        OPEN(20,FILE='..\OUTPUT\ZUV.OUT')
        OPEN(24,FILE='..\OUTPUT\SHA.OUT')
        OPEN(28,FILE='..\OUTPUT\H2U2V2.OUT')
        END IF
        OPEN(901,FILE='..\OUTPUT\XY-TEC.DAT')       !输出tecplot
        OPEN(902,FILE='..\OUTPUT\SHA-TEC.DAT')       !输出tecplot
        CALL OUTPUT2
...
C        
          DO 800 KT=1,K0        
            IF(NZQM==1)CALL TWOD                              !计算水流
            IF(NWQ==1)CALL TWODWQ                             !计算水质 
            IF(NSED==1)CALL SEDIMENTS                         !计算泥沙           
...
*************************************************************************初始条件设置        
       BLOCK DATA                                                  
        ...
        DATA M1SORT,M2SORT,M3SORT/0,2,15/                           !区分冲泄质和悬移质
               
       END
C#####################################################################        
C#################################################################
C       SUBROUTINE TAKING IN BOUNDARY CONDITIONS FOR              边界条件读入
C       2-D CALCULATION
C#################################################################

C#####################################################################
        SUBROUTINE PRE2                                     !前处理文件
...
C
        ...
        CALL PRECOR                 !通过计算下一时步的FLUX(K,J,I)，进而计算下一时步的WH\WU\WV
...
C       FLUX BALANCE FOR 2D CELLS
        ...
        DATA GMAR/1690/
        DATA SK,SM/0.08,0.5/   !SK=0.01                   SK 挟沙力系数
        DATA FKN/0.010/
C
