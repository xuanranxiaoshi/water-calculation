...
#define READ(FILE, X)       \
  ...
  X = readFromLine(line);
#define PATH_CREATE(X) std::string path = data_root_path + X
...
int readFromLine(std::string line)
{
...
void read_time()
{
  PATH_CREATE("/TIME.DAT");
  std::ifstream fileTime(path);
  ASSERT_READ(fileTime);
  std::string line;
  READ(fileTime, MDT)
  READ(fileTime, NDAYS)
  fileTime.close();
...
void read_boundary()
{
  ...
  std::string line;
  READ(fileBoundary, NZ)
  READ(fileBoundary, NQ)
  // READ(fileBoundary, NZQ)
  READ(fileBoundary, dummy)
  READ(fileBoundary, NHQ)
  // READ(fileBoundary, NWE)
  READ(fileBoundary, dummy)
  READ(fileBoundary, NDI)
  fileBoundary.close();
}
...
void pre2()
{
  ...
  for (int i = 0; i < CEL; i++)
  {
    ...
    double YP2 = YP[NW[1]];
    double YP3 = YP[NW[2]];
    AREA[i] = (XP1 * YP2 - YP1 * XP2 - XP1 * YP3 + YP1 * XP3 + XP2 * YP3 - YP2 * XP3) / 2.0;
    if (NV[i] == 4)
    {
      double XP4 = XP[NW[3]];
      double YP4 = YP[NW[3]];
      AREA[i] = (XP1 * YP3 - YP1 * XP3 - XP1 * YP4 + YP1 * XP4 + XP3 * YP4 - YP3 * XP4) / 2.0;
    }
    for (int j = 0; j < NV[i]; j++)
    {
      int N1 = NW[j];
...
void take_boundary_for_two_d()
{
  ...
  for (K = 1; K <= NNZ0; K++)
  {
    ...
    FILE *file = fopen(("..\\BOUNDE\\NZ\\NZ" + std::string(pointname) + ".dat").c_str(), "r");
    if (file == NULL)
    {
      // 错误处理
      exit(EXIT_FAILURE);
  ...
  {
    sprintf(pointname, "%04d", K);
    FILE *file = fopen(("..\\BOUNDE\\NQ\\NQ" + std::string(pointname) + ".dat").c_str(), "r");
    if (file == NULL)
    {
      // 错误处理
      exit(EXIT_FAILURE);
...
void BOUNDRYinterp(double THOURS, double *ZQSTEMP1, int NZQSTEMP, Vec ZQSTIME, Vec ZQSTEMP)
{
  int I;
  ...
  for (I = 1; I < NZQSTEMP; I++)
  {
    ...
    {
      *ZQSTEMP1 = ZQSTEMP[I] + (ZQSTEMP[I + 1] - ZQSTEMP[I]) / (ZQSTIME[I + 1] - ZQSTIME[I]) * (THOURS - ZQSTIME[I]);
    }
  }
}
